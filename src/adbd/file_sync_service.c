...
static int handle_send_file(int s,char*path,mode_t mode,char*buffer){
	...
	for(;;){
		...
		if(msg.data.id !=ID_DATA){
			if(msg.data.id==ID_DONE){timestamp=ltohl(msg.data.size);break;}
			fail_message(s,"invalid data message");
			goto fail;
		}
		len=ltohl(msg.data.size);
		if(len > SYNC_DATA_MAX){fail_message(s,"oversize data message");goto fail;}
...
static int handle_send_link(int s,char*path,char*buffer){
	...
	if(msg.data.id !=ID_DATA){fail_message(s,"invalid data message: expected ID_DATA");return -1;}
	if((len=ltohl(msg.data.size))> SYNC_DATA_MAX){fail_message(s,"oversize data message");return -1;}
	if(readx(s,buffer,len))return -1;
	if((ret=symlink(buffer,path))&&errno==ENOENT){mkdirs(path);ret=symlink(buffer,path);}
