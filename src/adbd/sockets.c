...
static int local_socket_enqueue(asocket*s,apacket*p){
	...
	if(p->len==0){put_apacket(p);return 0;}
enqueue:
	p->next=0;
...
static void local_socket_destroy(asocket*s){
	...
	for(p=s->pkt_first;p;p=n){n=p->next;put_apacket(p);}
	remove_socket(s);
	free(s);
...
static void local_socket_close_locked(asocket*s){
	...
	if(s->closing||s->pkt_first==NULL){local_socket_destroy(s);return;}
	s->closing=1;
...
static void local_socket_event_func(int fd,unsigned ev,void*_s){
	...
	if(ev&FDE_READ){
		...
		if((avail==MAX_PAYLOAD)||(s->peer==0))put_apacket(p);
		else{
...
asocket*create_local_service_socket(const char*name){
	asocket*s;
	int fd;
	if(!strcmp(name,"jdwp"))return create_jdwp_service_socket();
	if(!strcmp(name,"track-jdwp"))return create_jdwp_tracker_service_socket();
...
void connect_to_remote(asocket*s,const char*destination){
	...
	if(len>(MAX_PAYLOAD-1))fatal("adbd: destination oversized");
...
static int smart_socket_enqueue(asocket*s,apacket*p){
	...
	if(s->pkt_first==0){
		...
		if((s->pkt_first->len + p->len)>MAX_PAYLOAD){put_apacket(p);goto fail;}
...
static void smart_socket_close(asocket*s){
	if(s->pkt_first)put_apacket(s->pkt_first);
...
void connect_to_smartsocket(asocket*s){asocket*ss=create_smart_socket(smart_socket_action);s->peer=ss;ss->peer=s;s->ready(s);}
